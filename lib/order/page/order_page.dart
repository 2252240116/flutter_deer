...
class _OrderPageState extends State<OrderPage>
  ...
  Future<void> _onPageChange(int index) async {
    provider.setIndex(index);//状态管理 选择哪个tabbarview 更新给tabbarview
    ...
    return Stack(
      children: <Widget>[
        Container(
          ...
          child: Column(
            children: <Widget>[
              /// 使用context.select替代Consumer  value.index选中的 index tabview自己的index
              LoadAssetImage(
                context.select<OrderPageProvider, int>((value) => value.index) == index ? imgList[index][0]
                    : imgList[index][1],
                width: 24.0,
                height: 24.0,
              ),
